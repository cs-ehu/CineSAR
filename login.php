...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<div class="rightdiv">
			<?php
				...
				if(isset($_SESSION['userlog']) && !empty($_SESSION['userlog'])) {
					echo "<script type=\"text/javascript\">alert('¡No puede ir a la página de iniciar sesión ya logueado!');window.location.replace(\"index.php\");</script>";
				}				
			?>
	...
	<body>
		<fieldset id="reg" style="border: 1px solid rgb(0,0,0); width: 400px; margin:auto; padding: 2%;">
			<legend class="regis" style="color: white; background-color: black;">Iniciar Sesión</legend>
			<form method="post" style="text-align: center;" onsubmit="validarLog();">
				<div>
					<label>Email</label>
				</div>
				<div>
					<input type="text" name="emaill" id="email"/>
				</div>
				<div>
					<label>Password</label>
				</div>
				<div>
					<input type="password" name="passl" id="pass"/>
				</div>
				<div class="paddbut">
